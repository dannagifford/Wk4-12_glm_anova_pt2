...
Let's run through the ANCOVA example covered in the video above. First we need to load the packages we need. We'll be using the `tidyverse` for general data wrangling and data visualisation, and then the `afex` package for building our ANCOVA model, and the `emmeans` package for running pairwise comparisons and displaying our adjusted means.  
